...
saveRDS(auc_numbers_merged,"Results_tumor_focused_analysis/auc_numbers_merged_filt.rds")
...
ggplot(auc_numbers_merged, aes(patient_id, fill=sample_id,y = PID_HIF1_TFPATHWAY))+
  ...
  stat_compare_means(size = 4, method = "wilcox.test", 
                     label = "p.format")+
...
ggsave("Results_tumor_focused_analysis/HIF_score_per_pat.pdf", height=4.5, width=8)

write.csv(auc_numbers_merged,"Results_tumor_focused_analysis/Tumor_AUC_Specific_Pathways.csv")
...
ggplot(auc_numbers_merged, aes(patient_id, fill=sample_id,y = REACTOME_ANTIGEN_PRESENTATION_FOLDING_ASSEMBLY_AND_PEPTIDE_LOADING_OF_CLASS_I_MHC))+
  ...
  scale_fill_manual(values=c(DB_Tumor_Tcell="#825d9e",
                             SG_Singlets = "#7cd3f7"))+xlab("")
ggsave("Results_tumor_focused_analysis/MHC_score_per_pat.pdf", height=4.5, width=8)

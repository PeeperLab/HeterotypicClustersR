##################################################################
# T cells - statistical enrichment analysis per pat
##################################################################
...
per_pat_enrichment=function(OBJ){
  ...
  md_split=lapply(md_split, function(x){
    setDT(x)
    x <- x[, .(pheno_tot = .N), by = c("patient_id", "sample_id", "annotated_clusters_labels_final")]
    y=x[,lapply(.SD, mean), .SDcols="pheno_tot",
        by=c("annotated_clusters_labels_final", "patient_id", "sample_id")]
  ...
  annotated_clusters_labels_final <- unique(md_split_dt$annotated_clusters_labels_final)
  patient_ids <- unique(md_split_dt$patient_id)
  all_combinations <- expand.grid(sample_id = sample_ids,patient_id = patient_ids, annotated_clusters_labels_final = annotated_clusters_labels_final)
  complete_data <- merge(all_combinations, md_split_dt[,.(sample_id,patient_id,annotated_clusters_labels_final,pheno_tot)],
                         by = c("sample_id","patient_id", "annotated_clusters_labels_final"), all.x = TRUE)
  ...
  for (pat in unique(complete_data$patient_id)){
    for (sam in c("DB_Tumor_Tcell", "DB_APC_Tcell")){
      for (top in unique(complete_data$annotated_clusters_labels_final)){
        y=complete_data[patient_id == pat,]
        y0=y[sample_id %in% c("SG_Singlets",sam),]
        y0[, tot_pat:= sum(pheno_tot)]
        y1=y0[annotated_clusters_labels_final ==top,]
...
fwrite(tcell_res, "Results_stats/Tcell_per_pat_stats.txt")
